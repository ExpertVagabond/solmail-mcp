...
impl Config {
    fn new() -> Self {
        ...
        let rpc_url = std::env::var("SOLANA_RPC_URL").unwrap_or_else(|_| format!("https://api.{network}.solana.com"));
        Self {
            api_url: std::env::var("SOLMAIL_API_URL").unwrap_or_else(|_| "https://solmail.online/api".to_string()),
            network,
            rpc_url,
            private_key: std::env::var("SOLANA_PRIVATE_KEY").ok(),
            merchant_wallet: std::env::var("MERCHANT_WALLET").unwrap_or_else(|_| "B5daxcMG9LgcXkZwuxBhHtuYxzG9J4ekgz1wUiMXw3xp".to_string()),
            client: reqwest::Client::new(),
        }
    }

    fn has_wallet(&self) -> bool { self.private_key.is_some() }

    fn wallet_address(&self) -> Result<String, String> {
        let pk = self.private_key.as_ref().ok_or("Wallet not configured. Set SOLANA_PRIVATE_KEY.")?;
        let decoded = bs58::decode(pk).into_vec().map_err(|e| format!("Invalid key: {e}"))?;
        if decoded.len() < 64 { return Err("Key must be 64 bytes".to_string()); }
        Ok(bs58::encode(&decoded[32..]).into_string())
    }

    async fn sol_price(&self) -> f64 {
        let resp = self.client.get("https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd")
            .send().await.ok();
        if let Some(r) = resp {
    ...
    async fn get_balance(&self) -> Result<Value, String> {
        ...
        let resp = self.client.post(&self.rpc_url).json(&body).send().await.map_err(|e| format!("{e}"))?;
        let val: Value = resp.json().await.map_err(|e| format!("{e}"))?;
        let lamports = val["result"]["value"].as_u64().unwrap_or(0);
        let sol = lamports as f64 / LAMPORTS_PER_SOL as f64;
        Ok(json!({"address": addr, "balance": sol, "balanceLamports": lamports, "network": self.network}))
    }
}

fn mail_price(country: &str) -> f64 {
    if country.to_uppercase() == "US" { 1.50 } else { 2.50 }
}
...
async fn call_tool(cfg: &Config, name: &str, args: &Value) -> Result<Value, String> {
    match name {
        ...
        "send_mail" => {
            if !cfg.has_wallet() { return Err("Wallet not configured. Set SOLANA_PRIVATE_KEY.".to_string()); }
            ...
            Ok(json!({"success": true, "result": result, "payment": {"amount": sol_amount, "priceUsd": total}}))
        }
...
async fn main() {
    ...
    loop {
        ...
        let response = match req.method.as_str() {
            "initialize" => json!({"jsonrpc":"2.0","id":req.id,"result":{"protocolVersion":"2024-11-05","capabilities":{"tools":{}},"serverInfo":{"name":"solmail-mcp","version":"0.1.0"}}}),
